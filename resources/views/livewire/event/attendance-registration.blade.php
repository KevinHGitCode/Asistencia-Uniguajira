<div>
    ...
    ══════════════════════════════════════════════════════════════════ --}}
    @if (in_array($step, ['search', 'select_type', 'select_program']) || $activeTab === 'participante')
    ...
    ══════════════════════════════════════════════════════════════════ --}}
    @if ($activeTab === 'participante')
        @php
            $inputCls  = 'w-full rounded-xl border border-neutral-300 bg-white px-3.5 py-2.5
                           text-sm text-gray-900 placeholder-gray-400 transition
                           focus:border-[#62a9b6] focus:outline-none focus:ring-2 focus:ring-[#62a9b6]/25
                           dark:border-zinc-600 dark:bg-zinc-700 dark:text-gray-100
                           dark:placeholder-zinc-500 dark:focus:border-[#62a9b6]';
            $labelCls  = 'mb-1 block text-xs font-medium text-gray-600 dark:text-gray-400';
            $errorCls  = 'mt-1 flex items-center gap-1 text-xs text-red-500 dark:text-red-400';
        @endphp
        ...
        <div wire:transition
            ...
            {{-- Header --}}
            <div class="border-b border-gray-100 px-6 pt-6 pb-4 dark:border-zinc-700">
                <div class="flex items-center gap-2.5">
                    <div class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl"
                         style="background: rgba(98,169,182,.12)">
                        <svg class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                             stroke="currentColor" stroke-width="2" style="color: #62a9b6">
                            <path stroke-linecap="round" stroke-linejoin="round"
                                  d="M18 7.5v3m0 0v3m0-3h3m-3 0h-3m-2.25-4.125a3.375 3.375 0 1 1-6.75 0 3.375 3.375 0 0 1 6.75 0ZM3 19.235v-.11a6.375 6.375 0 0 1 12.75 0v.109A12.318 12.318 0 0 1 9.374 21c-2.331 0-4.512-.645-6.374-1.766Z"/>
                        </svg>
                    </div>
                    <div>
                        <h2 class="text-sm font-bold text-gray-800 dark:text-gray-100">Registrar participante</h2>
                        <p class="text-xs text-gray-400 dark:text-zinc-500">Nuevo en el sistema</p>
                    </div>
                </div>
            </div>
                ...
                <div class="space-y-3">
                    ...
                    <div>
                        ...
                        <input
                            ...
                            inputmode="numeric"
                            placeholder="Ej: 1073456789"
                            class="{{ $inputCls }} {{ $errors->has('newDocument') ? 'border-red-400 focus:ring-red-300/40' : '' }}"
                        />
                        @error('newDocument')
                            <p class="{{ $errorCls }}">
                    ...
                    <div class="grid grid-cols-2 gap-2">
                        <div>
                            ...
                            <input
                                ...
                                type="text"
                                placeholder="Ej: Ana"
                                class="{{ $inputCls }} {{ $errors->has('newFirstName') ? 'border-red-400 focus:ring-red-300/40' : '' }}"
                            />
                            @error('newFirstName')
                                <p class="{{ $errorCls }}">{{ $message }}</p>
                            @enderror
                        ...
                        <div>
                            ...
                            <input
                                ...
                                type="text"
                                placeholder="Ej: García"
                                class="{{ $inputCls }} {{ $errors->has('newLastName') ? 'border-red-400 focus:ring-red-300/40' : '' }}"
                            />
                            @error('newLastName')
                                <p class="{{ $errorCls }}">{{ $message }}</p>
                            @enderror
                    ...
                    <div>
                        ...
                        <input
                            ...
                            class="{{ $inputCls }} {{ $errors->has('newEmail') ? 'border-red-400 focus:ring-red-300/40' : '' }}"
                        />
                        @error('newEmail')
                            <p class="{{ $errorCls }}">{{ $message }}</p>
                        @enderror
                    ...
                    {{-- Afiliación (solo si rol = Docente) --}}
                    @if ($newRole === 'Docente')
                        <div wire:transition>
                    ...
                    {{-- Programa --}}
                    <div>
                        <label class="{{ $labelCls }}">Programa académico</label>
                        <select wire:model="newProgramId" class="{{ $inputCls }}">
                            <option value="">— Seleccionar —</option>
                    ...
                    {{-- Separador --}}
                    <div class="border-t border-dashed border-gray-200 pt-1 dark:border-zinc-700">
                        <p class="text-[10px] font-semibold uppercase tracking-widest text-gray-400 dark:text-zinc-500">
                            Datos opcionales
                        </p>
                    </div>
                    ...
                    <div>
                        ...
                        <select wire:model="newSexo" class="{{ $inputCls }}">
                            <option value="">— Seleccionar —</option>
                            @foreach (\App\Livewire\Event\AttendanceRegistration::SEXO_OPCIONES as $opcion)
                                <option value="{{ $opcion }}">{{ $opcion }}</option>
                            @endforeach
                    ...
                    {{-- Grupo priorizado --}}
                    <div>
                        <label class="{{ $labelCls }}">Grupo priorizado</label>
                        <select wire:model="newGrupoPriorizado" class="{{ $inputCls }}">
                            <option value="">— Seleccionar —</option>
                            @foreach (\App\Livewire\Event\AttendanceRegistration::GRUPOS_PRIORIZADOS as $grupo)
                                <option value="{{ $grupo }}">{{ $grupo }}</option>
                            @endforeach
            ...
            <div class="border-t border-gray-100 px-5 py-4 dark:border-zinc-700">
                <button
                    wire:click="registerParticipant"
                    wire:loading.attr="disabled"
                    wire:target="registerParticipant"
                    type="button"
                    class="w-full rounded-xl py-3.5 px-6 text-base font-bold text-white shadow
                           transition-all duration-200 hover:opacity-90 active:scale-[.98]
                           disabled:cursor-not-allowed disabled:opacity-60"
                    style="background: linear-gradient(90deg, #62a9b6 0%, #4d94a0 100%)">
                    <span wire:loading.remove wire:target="registerParticipant"
                          class="flex items-center justify-center gap-2">
                        <svg class="h-4 w-4" fill="none" viewBox="0 0 24 24"
                             stroke="currentColor" stroke-width="2.5">
                            <path stroke-linecap="round" stroke-linejoin="round"
                    ...
                    <span wire:loading wire:target="registerParticipant"
                          class="flex items-center justify-center gap-2">
                        <svg class="h-4 w-4 animate-spin" fill="none" viewBox="0 0 24 24">
                            <circle class="opacity-25" cx="12" cy="12" r="10"
                                    stroke="currentColor" stroke-width="4"/>
                ...
                <p class="mt-2 text-center text-xs text-gray-400 dark:text-zinc-500">
                    Al registrar, se procederá a marcar la asistencia al evento
                </p>
            </div>
        </div>
    @endif {{-- fin tab participante --}}
