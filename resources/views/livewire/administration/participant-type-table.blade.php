<div class="border border-neutral-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm overflow-hidden">
    ...
    <div class="px-4 sm:px-6 py-4 border-b border-neutral-200 dark:border-neutral-700 bg-zinc-50 dark:bg-zinc-900 flex flex-wrap items-center justify-between gap-3">
        ...
        <div class="flex items-center gap-3 ml-auto">
            @if($participantTypes->hasPages())
            <div class="flex items-center gap-0.5">
                <button wire:click="previousPage"
                        wire:loading.attr="disabled"
                        wire:target="previousPage,nextPage,gotoPage"
                        @disabled($participantTypes->onFirstPage())
                        class="p-1.5 rounded-lg transition-colors {{ $participantTypes->onFirstPage() ? 'text-gray-300 dark:text-zinc-600 cursor-not-allowed' : 'text-gray-500 dark:text-gray-400 hover:bg-zinc-200 dark:hover:bg-zinc-700 cursor-pointer' }}"
                        title="Página anterior">
                    <flux:icon.chevron-left class="size-4" />
                </button>

                <span class="text-xs font-medium text-gray-500 dark:text-gray-400 px-1.5 tabular-nums select-none">
                    {{ $participantTypes->currentPage() }}&thinsp;/&thinsp;{{ $participantTypes->lastPage() }}
                </span>

                <button wire:click="nextPage"
                        wire:loading.attr="disabled"
                        wire:target="previousPage,nextPage,gotoPage"
                        @disabled(!$participantTypes->hasMorePages())
                        class="p-1.5 rounded-lg transition-colors {{ !$participantTypes->hasMorePages() ? 'text-gray-300 dark:text-zinc-600 cursor-not-allowed' : 'text-gray-500 dark:text-gray-400 hover:bg-zinc-200 dark:hover:bg-zinc-700 cursor-pointer' }}"
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-neutral-100 dark:divide-zinc-800">
                @forelse($participantTypes as $type)
                    <tr wire:key="participant-type-{{ $type->id }}" class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                        <td class="px-4 sm:px-6 py-4 text-gray-400 dark:text-zinc-500 font-mono text-xs">
                            {{ ($participantTypes->currentPage() - 1) * $participantTypes->perPage() + $loop->iteration }}
                        </td>
    ...
    @if($participantTypes->hasPages())
    @php
        $current = $participantTypes->currentPage();
        $last = $participantTypes->lastPage();
    ...
    <div class="px-4 sm:px-6 py-4 border-t border-neutral-100 dark:border-zinc-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <p class="text-xs text-gray-500 dark:text-gray-400">
            Mostrando
            <span class="font-medium text-gray-700 dark:text-gray-300">{{ $participantTypes->firstItem() }}</span>–<span class="font-medium text-gray-700 dark:text-gray-300">{{ $participantTypes->lastItem() }}</span>
            de
            <span class="font-medium text-gray-700 dark:text-gray-300">{{ $participantTypes->total() }}</span>
            {{ Str::plural('estamento', $participantTypes->total()) }}
        </p>

        <div class="flex items-center gap-1">
            @if($participantTypes->onFirstPage())
            ...
            @for($p = $left; $p <= $right; $p++)
                @if($p === $current)
                    <span class="w-8 h-8 flex items-center justify-center rounded-lg text-xs font-semibold text-white bg-[#0d9488] select-none">{{ $p }}</span>
                @else
            ...
            @if($participantTypes->hasMorePages())
                <button wire:click="nextPage"
                        wire:loading.attr="disabled"
                        wire:target="previousPage,nextPage,gotoPage"
                        class="px-2.5 py-1.5 rounded-lg text-xs font-medium text-gray-600 dark:text-gray-300 hover:bg-zinc-100 dark:hover:bg-zinc-700 transition-colors cursor-pointer">
