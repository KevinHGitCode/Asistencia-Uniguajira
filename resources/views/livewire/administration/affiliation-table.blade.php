<div class="border border-neutral-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div class="flex items-center gap-3 ml-auto">
            ...
            <div class="flex items-center gap-0.5">
                <button wire:click="previousPage"
                        ...
                        @disabled($affiliations->onFirstPage())
                        class="p-1.5 rounded-lg transition-colors {{ $affiliations->onFirstPage() ? 'text-gray-300 dark:text-zinc-600 cursor-not-allowed' : 'text-gray-500 dark:text-gray-400 hover:bg-zinc-200 dark:hover:bg-zinc-700 cursor-pointer' }}"
                        title="Página anterior">
                    <flux:icon.chevron-left class="size-4" />
                </button>

                <span class="text-xs font-medium text-gray-500 dark:text-gray-400 px-1.5 tabular-nums select-none">
                    {{ $affiliations->currentPage() }}&thinsp;/&thinsp;{{ $affiliations->lastPage() }}
                </span>

                <button wire:click="nextPage"
                        wire:loading.attr="disabled"
                        wire:target="previousPage,nextPage,gotoPage"
                        @disabled(!$affiliations->hasMorePages())
                        class="p-1.5 rounded-lg transition-colors {{ !$affiliations->hasMorePages() ? 'text-gray-300 dark:text-zinc-600 cursor-not-allowed' : 'text-gray-500 dark:text-gray-400 hover:bg-zinc-200 dark:hover:bg-zinc-700 cursor-pointer' }}"
        ...
        <table class="w-full text-sm">
            ...
            <tbody class="divide-y divide-neutral-100 dark:divide-zinc-800">
                @forelse($affiliations as $affiliation)
                    <tr wire:key="affiliation-{{ $affiliation->id }}" class="hover:bg-zinc-50 dark:hover:bg-zinc-800/50 transition-colors">
                        <td class="px-4 sm:px-6 py-4 text-gray-400 dark:text-zinc-500 font-mono text-xs">
                            {{ ($affiliations->currentPage() - 1) * $affiliations->perPage() + $loop->iteration }}
                        </td>
                        ...
                        <td class="px-4 sm:px-6 py-4">
                            <div class="flex items-center justify-end gap-2">
                                <button
                                    @click="openEdit({{ $affiliation->id }}, {{ Js::from($affiliation->name) }})"
                                ...
                                @if(($affiliation->participants_count ?? 0) === 0)
                                    <button
                                        @click="openDelete({{ $affiliation->id }}, {{ Js::from($affiliation->name) }})"
                    ...
                    <tr>
                        <td colspan="5" class="px-6 py-16 text-center">
                            <div class="flex flex-col items-center gap-3 text-gray-400 dark:text-zinc-500">
                                ...
                                <p class="text-sm">
                                    @if($search)
                                        No se encontraron afiliaciones para
                                        <span class="font-medium text-gray-600 dark:text-gray-300">"{{ $search }}"</span>.
                                    @else
                                        No hay afiliaciones registradas aún.
                                    @endif
                                </p>
                                @unless($search)
    ...
    @if($affiliations->hasPages())
    @php
        $current = $affiliations->currentPage();
        $last = $affiliations->lastPage();
    ...
    <div class="px-4 sm:px-6 py-4 border-t border-neutral-100 dark:border-zinc-800 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <p class="text-xs text-gray-500 dark:text-gray-400">
            Mostrando
            <span class="font-medium text-gray-700 dark:text-gray-300">{{ $affiliations->firstItem() }}</span>–<span class="font-medium text-gray-700 dark:text-gray-300">{{ $affiliations->lastItem() }}</span>
            de
            <span class="font-medium text-gray-700 dark:text-gray-300">{{ $affiliations->total() }}</span>
            {{ Str::plural('afiliación', $affiliations->total()) }}
        </p>

        <div class="flex items-center gap-1">
            @if($affiliations->onFirstPage())
            ...
            @for($p = $left; $p <= $right; $p++)
                @if($p === $current)
                    <span class="w-8 h-8 flex items-center justify-center rounded-lg text-xs font-semibold text-white bg-[#7c6fcd] select-none">{{ $p }}</span>
                @else
