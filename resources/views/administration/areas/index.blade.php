...
<div class="flex h-full w-full flex-1 flex-col gap-6 p-1 sm:p-4 md:p-6"
     x-data="{ ...areasManager(), activeTab: new URLSearchParams(window.location.search).get('tab') || '{{ session('active_tab', 'list') }}', setTab(tab) { this.activeTab = tab; const url = new URL(window.location); url.searchParams.set('tab', tab); window.history.replaceState({}, '', url); } }">
    ...
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        ...
        <button @click="openCreate()"
            class="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-[#62a9b6] text-white text-sm font-medium transition-colors shadow-sm cursor-pointer self-start sm:self-auto hover:opacity-90">
        ...
        <nav class="flex gap-1">
            <button
                ...
                :class="activeTab === 'list'
                    ? 'border-b-2 border-[#62a9b6] text-[#62a9b6] dark:text-[#8ecad4]'
    ...
    <div x-show="activeTab === 'import'" x-transition>
        <div class="border border-neutral-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 rounded-2xl shadow-sm overflow-hidden">
            ...
            <div class="px-4 sm:px-6 py-4 border-b border-neutral-200 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-900 flex items-center justify-between gap-4">
                <div>
                    ...
                    <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5">
                        Columnas requeridas: <code class="font-mono">Nombre</code> y <code class="font-mono">Dependencia</code>. Los duplicados se omiten automáticamente.
                    </p>
                </div>
                <a href="{{ route('areas.download-template') }}"
                   class="inline-flex items-center gap-1.5 px-3 py-2 rounded-lg border border-neutral-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-700 text-gray-700 dark:text-gray-300 text-xs font-medium transition-colors shrink-0">
                    <flux:icon.arrow-down-tray class="size-3.5 text-[#62a9b6]" />
                    Descargar plantilla
                </a>
            </div>
            ...
            <div class="px-4 sm:px-6 py-6 flex flex-col gap-6">
                ...
                {{-- Drop zone --}}
                <form action="{{ route('areas.import') }}" method="POST" enctype="multipart/form-data"
                    ...
                    <div
                        ...
                        :class="dragging ? 'border-[#62a9b6] bg-cyan-50 dark:bg-cyan-900/20' : 'border-neutral-300 dark:border-zinc-700 bg-zinc-50 dark:bg-zinc-800/50'"
                        ...
                        <div>
                            <p class="text-sm font-medium text-gray-700 dark:text-gray-300">
                                <span x-show="!fileName">Arrastra tu archivo aquí o <span class="text-[#62a9b6]">selecciona uno</span></span>
                                <span x-show="fileName" class="text-[#62a9b6]" x-text="fileName"></span>
                            </p>
                    ...
                    <div class="flex justify-end">
                        <button type="submit"
                            class="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-[#62a9b6] hover:opacity-90 text-white text-sm font-medium transition-colors shadow-sm cursor-pointer">
                            <flux:icon.arrow-up-tray class="size-4" />
                            Importar
                        </button>
                    </div>
                ...
                <div class="flex items-center justify-between gap-4">
                    ...
                    <a href="{{ route('areas.download-export') }}"
                       class="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg border border-neutral-200 dark:border-zinc-700 bg-white dark:bg-zinc-800 hover:bg-zinc-50 dark:hover:bg-zinc-700 text-gray-700 dark:text-gray-300 text-sm font-medium transition-colors shadow-sm shrink-0">
                        <flux:icon.arrow-down-tray class="size-4 text-[#62a9b6]" />
                        Descargar Excel
                    </a>
                </div>
            </div>
