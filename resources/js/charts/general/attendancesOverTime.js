...
export function renderAttendancesOverTime(charts) {
  ...
  fetch(getApiUrl('/api/statistics/attendances-over-time'))
    .then(res => res.json())
    .then(data => {
      if (!data || data.length === 0) {
        console.warn('No hay datos para attendancesOverTime');
        return;
      }
          ...
          formatter: (params) => {
            ...
            return `
              <div style="padding: 4px 0;">
                ...
                <div style="color: ${dark ? '#34d399' : '#059669'};">
                  <span style="font-size: 18px; font-weight: 700;">${p.data}</span>
                  <span style="font-size: 12px; margin-left: 4px;">asistencias</span>
                </div>
              </div>
            `;
          }
        ...
        dataZoom: [
          ...
          },
          {
            ...
            handleStyle: {
              ...
              borderColor: dark ? '#10b981' : '#047857'
            },
            textStyle: { color: dark ? '#fff' : '#333' }
          }
        ],
        series: [
          {
            ...
            areaStyle: {
              color: dark ? 'rgba(52,211,153,0.1)' : 'rgba(5,150,105,0.08)'
            },
            emphasis: {
              focus: 'series',
              itemStyle: {
                borderWidth: 3,
                shadowBlur: 10,
                shadowColor: dark ? 'rgba(52,211,153,0.5)' : 'rgba(5,150,105,0.3)'
              }
            }
          }
        ]
